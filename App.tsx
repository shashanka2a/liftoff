...
import { 
  ArrowRight, Layers, Zap, Globe, Cpu, BarChart3, 
  LayoutGrid, Settings, Folder, Search, Command, 
  Wifi, Battery, ChevronRight, Terminal, Activity, X,
  Maximize2, Minus, Power, MessageSquare, Share2,
  FileText, Image as ImageIcon, Grid, List, Info, Calendar, User, TrendingUp,
  CreditCard, ShoppingBag, Cloud, Smartphone, Moon, Sun, Monitor, Lock
} from 'lucide-react';

/**
 * LIFTOFF™ OS - Strategic Product Studio
 * V 3.5 - Strict Dependency Fix
 * - Re-ordered ALL components to ensure definition before usage.
 * - Fixed ReferenceError for WorkSection.
 * - Sanitized props to prevent Object-as-Child errors.
 */

// ==========================================
// 1. UTILS & HOOKS (Level 0)
// ==========================================
...
// ==========================================
// 2. ATOMIC UI COMPONENTS (Level 1)
// ==========================================
...
const TrafficLights = ({ onClose, onMinimize, onMaximize }) => (
  <div className="flex gap-2 group">
    <button onClick={onClose} className="w-3 h-3 rounded-full bg-[#FF5F57] border border-[#E0443E]/50 shadow-inner flex items-center justify-center hover:bg-[#FF5F57]/80 transition-colors group/btn">
      <X className="w-2 h-2 text-black/50 opacity-0 group-hover/btn:opacity-100" />
    </button>
    <button onClick={onMinimize} className="w-3 h-3 rounded-full bg-[#FEBC2E] border border-[#D89E24]/50 shadow-inner flex items-center justify-center hover:bg-[#FEBC2E]/80 transition-colors group/btn">
      <Minus className="w-2 h-2 text-black/50 opacity-0 group-hover/btn:opacity-100" />
    </button>
    <button onClick={onMaximize} className="w-3 h-3 rounded-full bg-[#28C840] border border-[#1AAB29]/50 shadow-inner flex items-center justify-center hover:bg-[#28C840]/80 transition-colors group/btn">
...
// ==========================================
// 3. COMPLEX UI COMPONENTS (Level 2)
// ==========================================

const WindowFrame = ({ children, className = "", windowState, setWindowState, url = "liftoff.design", showLoadingBar = false }) => {
  ...
  if (isClosed) return (
    <div className="h-[70vh] flex flex-col items-center justify-center border border-black/5 dark:border-white/5 rounded-[32px] bg-black/[0.02] dark:bg-white/[0.02]">
      <div className="text-zinc-400 dark:text-zinc-500 mb-4 font-mono text-sm">SYSTEM_HALTED</div>
      <button 
      ...
      <div className="h-12 min-h-[48px] border-b border-black/5 dark:border-white/[0.06] flex items-center justify-between px-4 md:px-6 z-50 select-none backdrop-blur-xl relative shrink-0">
        ...
        <div className="w-1/3 flex justify-center">
          <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-black/5 dark:bg-black/20 rounded-lg border border-black/5 dark:border-white/5 text-[11px] text-zinc-500 dark:text-zinc-400 font-mono shadow-inner w-full max-w-[340px] justify-center cursor-default group hover:bg-black/10 dark:hover:bg-black/30 transition-colors">
            ...
            <span className="group-hover:text-zinc-800 dark:group-hover:text-zinc-300 transition-colors truncate">{url}</span>
          </div>
          <div className="md:hidden text-zinc-500 font-bold tracking-widest text-[10px]">LIFTOFF™</div>
        </div>

        <div className="w-1/3 flex justify-end gap-2">
           {isMaximized && (
             <button onClick={() => setWindowState('normal')} className="p-2 hover:bg-black/5 dark:hover:bg-white/10 rounded-lg transition-colors">
        ...
        {showLoadingBar && (
          <div className="absolute bottom-0 left-0 h-[2px] bg-blue-500 z-50 animate-[width_1.5s_ease-in-out_forwards] w-0 shadow-[0_0_10px_rgba(59,130,246,0.5)]" style={{ width: '100%' }} />
        )}
      </div>
...
const SettingsWindow = ({ onClose, theme, setTheme }) => {
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center px-4 pointer-events-none">
      ...
      <div className={`
        ...
        shadow-2xl shadow-black/20 dark:shadow-black/50 pointer-events-auto overflow-hidden animate-in fade-in zoom-in-95 duration-200
      `}>
        ...
        <div className="p-6">
          <div className="mb-4">
            <h3 className="text-sm font-medium text-zinc-900 dark:text-white mb-1">Appearance</h3>
            <p className="text-xs text-zinc-500">Select your preferred system theme.</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <button 
              className="p-3 rounded-xl border border-black/5 dark:border-white/10 opacity-50 cursor-not-allowed flex flex-col items-center gap-2 text-zinc-400"
            >
...
const FilePreviewWindow = ({ file, onClose }) => {
  ...
  return (
    <div className="absolute inset-0 z-[250] flex items-center justify-center px-4 md:px-8 py-8 pointer-events-none">
       {/* Backdrop */}
       <div 
        className={`absolute inset-0 bg-white/20 dark:bg-black/40 backdrop-blur-md transition-opacity duration-300 ${isClosing ? 'opacity-0' : 'opacity-100'} pointer-events-auto`} 
        onClick={handleClose}
      />
      ...
      <div className={`
        ...
        ${isClosing ? 'scale-95 opacity-0 translate-y-4' : 'scale-100 opacity-100 translate-y-0 animate-in fade-in zoom-in-95 slide-in-from-bottom-4'}
      `}>
        ...
        <div className="flex-1 bg-white dark:bg-[#0A0A0A] relative flex items-center justify-center p-8 border-r border-black/5 dark:border-white/5">
           ...
           <div className="relative w-full h-full max-w-4xl max-h-[70vh] shadow-2xl rounded-lg overflow-hidden ring-1 ring-black/5 dark:ring-white/10">
              {file.type === 'image' ? (
                <img src={file.src} alt={file.name} className="w-full h-full object-contain bg-zinc-50 dark:bg-[#111]" />
              ) : (
        ...
        <div className="w-80 bg-[#FAFAFA] dark:bg-[#141414] flex flex-col">
          ...
          <div className="h-14 border-b border-black/5 dark:border-white/5 flex items-center justify-between px-6">
            <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Project Details</span>
            <button onClick={handleClose} className="p-2 hover:bg-black/5 dark:hover:bg-white/10 rounded-lg transition-colors">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8">
            ...
            <div>
              <div className="w-16 h-16 rounded-xl bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center border border-blue-100 dark:border-blue-500/20 mb-4">
                {file.type === 'image' ? <ImageIcon className="w-8 h-8 text-blue-500 dark:text-blue-400" /> : <FileText className="w-8 h-8 text-blue-500 dark:text-blue-400" />}
              </div>
              <h3 className="text-xl font-medium text-zinc-900 dark:text-white mb-1 leading-snug break-words">{file.name}</h3>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between text-sm py-2 border-b border-black/5 dark:border-white/5">
                <span className="text-zinc-500 flex items-center gap-2"><User className="w-3.5 h-3.5" /> Client</span>
                <span className="text-zinc-800 dark:text-zinc-300 font-medium text-xs">{file.client}</span>
              </div>
              <div className="flex items-center justify-between text-sm py-2 border-b border-black/5 dark:border-white/5">
                <span className="text-zinc-500 flex items-center gap-2"><Layers className="w-3.5 h-3.5" /> Scope</span>
                <span className="text-zinc-800 dark:text-zinc-300 font-medium text-xs">{file.scope}</span>
              </div>
              <div className="flex items-center justify-between text-sm py-2 border-b border-black/5 dark:border-white/5">
                <span className="text-zinc-500 flex items-center gap-2"><Calendar className="w-3.5 h-3.5" /> Year</span>
                <span className="text-zinc-800 dark:text-zinc-300 font-mono text-xs">{file.year}</span>
              </div>
              <div className="flex items-center justify-between text-sm py-2 border-b border-black/5 dark:border-white/5">
                <span className="text-zinc-500 flex items-center gap-2"><TrendingUp className="w-3.5 h-3.5" /> Outcome</span>
                <span className="text-emerald-600 dark:text-emerald-400 font-mono text-xs">{file.impact}</span>
              </div>
            </div>
...
// --- 4. Feature Components (Level 3) ---

const FolderWindow = ({ category, onClose }) => {
  const [isClosing, setIsClosing] = useState(false);
  ...
  const getMockData = (index, type = 'image') => {
    const clients = {
  ...
  return (
    <div className={`fixed inset-0 z-[200] flex items-center justify-center px-4 pointer-events-none`}>
      ...
      {/* Backdrop */}
      <div 
        className={`absolute inset-0 bg-white/40 dark:bg-black/60 backdrop-blur-sm transition-opacity duration-300 ${isClosing ? 'opacity-0' : 'opacity-100'} pointer-events-auto`} 
        onClick={handleClose}
      />
      ...
      `}>
        ...
        <div className="h-12 border-b border-black/5 dark:border-white/5 flex items-center justify-between px-4 bg-white/[0.02] relative z-10">
          <div className="flex items-center gap-4 w-1/3">
            <TrafficLights onClose={handleClose} onMinimize={handleClose} onMaximize={() => {}} />
          </div>
        ...
        <div className="flex-1 flex overflow-hidden relative">
          ...
          <div className="w-48 border-r border-black/5 dark:border-white/5 bg-white/40 dark:bg-black/20 p-4 hidden sm:block relative z-10 backdrop-blur-md">
            <div className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-4">Locations</div>
            <div className="space-y-1">
              {['Desktop', 'Documents', 'Downloads', 'AirDrop'].map(item => (
                <div key={item} className="px-3 py-1.5 rounded-lg text-sm text-zinc-600 dark:text-zinc-400 hover:bg-black/5 dark:hover:text-white dark:hover:bg-white/5 cursor-pointer flex items-center gap-2">
                   <Folder className="w-3.5 h-3.5" /> {item}
                </div>
              ))}
            </div>
            <div className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mt-6 mb-4">Tags</div>
            <div className="space-y-1">
          ...
          <div className="flex-1 p-6 overflow-y-auto custom-scrollbar relative z-10 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-100">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 relative z-10">
                ...
                return (
                  ...
                  >
                    <div className="aspect-square rounded-xl bg-white dark:bg-zinc-800 overflow-hidden border border-black/5 dark:border-white/5 mb-3 relative shadow-sm dark:shadow-lg group-hover:ring-2 ring-blue-500/50 transition-all">
                      <img src={img} alt={`Project ${i}`} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-3">
                         <ImageIcon className="w-4 h-4 text-white" />
                      </div>
                    </div>
                    <div className="text-center">
                      <div className="text-sm text-zinc-900 dark:text-white font-medium group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors truncate">{mockData.client}</div>
              ...
              {/* Dummy Files */}
              {[1,2,3].map((_, i) => {
                const mockData = getMockData(i + 3, 'doc');
                return (
                  <div 
                    key={`dummy-${i}`} 
                    className="group cursor-pointer"
                  ...
                  >
                    <div className="aspect-square rounded-xl bg-white/50 dark:bg-white/5 border border-black/5 dark:border-white/5 mb-3 flex items-center justify-center group-hover:bg-white dark:group-hover:bg-white/10 transition-colors shadow-sm">
                     ...
                     <div className="text-center">
                      <div className="text-sm text-zinc-600 dark:text-zinc-400 group-hover:text-zinc-900 dark:group-hover:text-white transition-colors truncate">{mockData.client}_Doc</div>
...
const FolderStack = ({ category, delay, onClick }) => {
  ...
  return (
    <FadeIn delay={delay}>
      <div 
        onClick={onClick}
        className="group relative w-full h-[450px] cursor-pointer perspective-1000"
      >
        <div className="relative w-full h-full flex items-center justify-center transition-transform duration-500">
          
          <div className="absolute w-[80%] aspect-[3/4] bg-zinc-200 dark:bg-zinc-800 rounded-2xl border border-black/5 dark:border-white/5 shadow-2xl transition-all duration-500 ease-out transform
          ...
          <div className="absolute w-[80%] aspect-[3/4] bg-white dark:bg-[#1A1A1A] rounded-2xl border border-black/5 dark:border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] transition-all duration-500 ease-out transform
          ...
          >
            ...
            <div className="space-y-4 relative z-10">
              <h3 className="text-3xl font-bold text-zinc-900 dark:text-white leading-tight">{category.name}</h3>
...
const SplashScreen = ({ onComplete }) => {
  ...
  const [stage, setStage] = useState('window-open');
  ...
  return (
    <div className={`fixed inset-0 z-[1000] flex items-center justify-center transition-opacity duration-700 bg-[#F2F2F2] dark:bg-[#050505] ${stage === 'fade-out' ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
      <div className={`
      ...
      `}>
        <WindowFrame 
          ...
          className="h-[75vh]"
        >
          {/* Clean loading state - Just window frame and loading bar */}
...
// ==========================================
// 5. MAIN SECTIONS (Level 4 - Dependent on UI Components)
// ==========================================

const Hero = ({ windowState, setWindowState }) => (
  <section id="hero" className="pt-24 pb-12 px-4 md:px-8 max-w-[1600px] mx-auto transition-all duration-500">
    <WindowFrame 
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full px-6 md:px-12 py-20 flex flex-col items-start justify-center h-full">
        <FadeIn delay={100} className="flex justify-start mb-10">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-black/5 dark:border-white/10 bg-white/30 dark:bg-white/5 backdrop-blur-md shadow-lg cursor-pointer hover:bg-white/50 dark:hover:bg-white/10 transition-colors">
            ...
            <span className="text-[10px] font-mono text-indigo-600 dark:text-indigo-200 uppercase tracking-wider">System v4.2 Online</span>
          </div>
        </FadeIn>
        ...
        <h1 className="text-6xl md:text-8xl lg:text-[120px] font-semibold tracking-tighter text-zinc-900 dark:text-white mb-8 leading-[0.9] text-left">
          ...
          <WordReveal text="Digital Products." delay={200} className="text-zinc-400 dark:text-white/50" />
        </h1>
        ...
        <FadeIn delay={600}>
          <p className="text-lg md:text-xl text-zinc-500 dark:text-[#A0A0A0] max-w-3xl leading-relaxed font-normal mb-12 text-left">
            ...
            We merge strategic branding with scalable engineering to build the infrastructure of tomorrow.
          </p>
        </FadeIn>
...
const WorkSection = ({ onOpenFolder }) => {
  const categories = [
    {
      name: "FinTech",
      count: "04",
      ...
      <FadeIn className="mb-16 flex items-end justify-between px-2">
        <div>
          <h2 className="text-3xl font-semibold text-zinc-900 dark:text-white flex items-center gap-3">
            Project Archives <span className="text-zinc-400 dark:text-zinc-600 font-light">/</span> <span className="text-zinc-400 dark:text-zinc-600 text-xl font-mono">DIR_ROOT</span>
          </h2>
        </div>
      </FadeIn>
...
const Features = () => (
  <section id="features" className="px-4 md:px-8 pb-32 max-w-[1600px] mx-auto">
    <FadeIn className="mb-12">
      <h2 className="text-3xl font-semibold text-zinc-900 dark:text-white flex items-center gap-3">
        Capabilities <span className="text-zinc-400 dark:text-zinc-600 font-light">/</span> <span className="text-zinc-400 dark:text-zinc-600 text-xl font-mono">SERVICES</span>
      </h2>
    </FadeIn>
        ...
        <FadeIn delay={0} className="h-full">
          <GlassCard className="h-full p-8 md:p-12 flex flex-col md:flex-row gap-12 justify-between group min-h-[400px]">
            ...
            <div className="relative z-10 flex-1 flex flex-col justify-center">
              ...
              <h3 className="text-4xl md:text-5xl font-medium text-zinc-900 dark:text-white mb-6">Product Design</h3>
              <p className="text-xl text-zinc-500 dark:text-zinc-400 leading-relaxed max-w-lg mb-8">
                We craft intuitive, human-centric interfaces. From complex design systems to high-fidelity prototyping, we bridge the gap between user needs and business goals.
              </p>
              <div className="flex flex-wrap gap-3">
                 {['UI/UX', 'Design Systems', 'Prototyping', 'User Research'].map(tag => (
                   <span key={tag} className="px-4 py-2 rounded-full bg-white/60 dark:bg-white/5 border border-black/5 dark:border-white/10 text-sm text-zinc-600 dark:text-zinc-300 font-mono">
                     {tag}
                   </span>
                 ))}
               </div>
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-blue-100/30 to-transparent dark:from-blue-500/5 dark:to-transparent" />
             ...
             <div className="flex-1 z-10">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <h3 className="text-3xl font-medium text-zinc-900 dark:text-white">Brand Identity</h3>
                </div>
                <p className="text-lg text-zinc-500 dark:text-zinc-400 max-w-md">Strategy, Logo Design, & Art Direction that cuts through the noise.</p>
             </div>
        ...
        <FadeIn delay={400} className="h-full">
           <GlassCard className="h-full p-10 flex flex-col justify-between hover:bg-white/80 dark:hover:bg-white/5 transition-colors min-h-[300px]">
              <div>
                ...
                <h4 className="text-3xl text-zinc-900 dark:text-white font-medium mb-2">Development</h4>
                <p className="text-zinc-500">Robust engineering for scalable applications.</p>
              </div>
              <div className="flex flex-wrap gap-2 mt-8">
                {['React', 'Next.js', 'WebGL', 'Node'].map(tag => (
                   <span key={tag} className="px-3 py-1 rounded bg-zinc-100 dark:bg-zinc-900 border border-black/5 dark:border-white/10 text-xs text-zinc-500 dark:text-zinc-400 font-mono">
                     {tag}
                   </span>
                ))}
              </div>
...
const Footer = () => (
  <footer id="footer" className="bg-[#FAFAFA] dark:bg-[#0A0A0A] border-t border-black/5 dark:border-white/5 pt-24 pb-48 px-6">
    <div className="max-w-[1600px] mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-20">
        <div className="col-span-1 md:col-span-2">
          <h2 className="text-2xl font-bold text-zinc-900 dark:text-white mb-6 tracking-wide">LIFTOFF™ STUDIO</h2>
        ...
        <div>
           ...
           <ul className="space-y-4 text-zinc-500 text-sm">
             <li className="hover:text-zinc-900 dark:hover:text-white cursor-pointer transition-colors">Work Index</li>
             <li className="hover:text-zinc-900 dark:hover:text-white cursor-pointer transition-colors">Capabilities</li>
             <li className="hover:text-zinc-900 dark:hover:text-white cursor-pointer transition-colors">Agency</li>
             <li className="hover:text-zinc-900 dark:hover:text-white cursor-pointer transition-colors">Careers <span className="text-xs bg-blue-500/10 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 px-1.5 py-0.5 rounded ml-2">Hiring</span></li>
           </ul>
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center pt-8 border-t border-black/5 dark:border-white/5 gap-4">
        ...
        <div className="flex gap-6 text-xs text-zinc-500">
           <span className="hover:text-zinc-800 dark:hover:text-zinc-400 cursor-pointer">Privacy Protocol</span>
           <span className="hover:text-zinc-800 dark:hover:text-zinc-400 cursor-pointer">Terms of Service</span>
        </div>
      </div>
    </div>
  </footer>
...
const Dock = ({ scrollToSection, onSettings }) => (
...
// ==========================================
// 6. MAIN APP COMPONENT (Level 5 - Root)
// ==========================================
  ...
  return (
    <div className={theme}>
      <div className="min-h-screen bg-[#F2F2F2] dark:bg-[#050505] text-zinc-900 dark:text-white font-sans selection:bg-indigo-500/30 transition-colors duration-500">
        ...
        {/* Main Content - Only visible after loading to prevent flash */}
        <main className={`relative z-10 w-full overflow-x-hidden transition-opacity duration-1000 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
          {/* Hero Section inside Window Frame */}
          <Hero windowState={windowState} setWindowState={setWindowState} />
          
          {/* Sections flowing on the desktop */}
          <Widgets />
